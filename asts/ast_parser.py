...
def get_syntax(source, root, node, lang=None):
    query = LANGUAGE[lang].query(PATTERNS_METHOD_INVOCATION[lang])
    captures = query.captures(root)
    captures = [str(cap[0]) for cap in captures]
    xsbt = []
    if len(node.children) == 0:
        if (str(node) not in captures):
            xsbt.append(node.type)
        else:
            xsbt.append(get_node_name(source=source, node=node, lang=lang))
    else:
        len_before = len(xsbt)
        for child in node.children:
            xsbt += get_syntax(source, root, child, lang)
        if len_before == len(xsbt) and len_before != 0:
            xsbt[-1] = node.type
    return xsbt

def get_xsbt(source, node, lang=None):
    xsbt = []
    if len(node.children) == 0:
        # print(node)
        # if get_node_name(source, node, lang=lang) not in ELIMINATE_AST_NODE and \
        #    node.type not in ELIMINATE_AST_NODE:
        if(node.type != 'identifier'):
            xsbt.append(node.type)
        else:
            xsbt.append(get_node_name(source, node, lang=lang))
    else:
        # print(node.type)
        xsbt.append(f'<{node.type}>'[:4]+'>')
        len_before = len(xsbt)
        for child in node.children:
            xsbt += get_xsbt(source, child, lang)
        if len_before == len(xsbt) and len_before != 0:
            xsbt[-1] = node.type
        else:
            xsbt.append(f'</{node.type}>'[:5]+'>')
    # xsbt = ' '.join(xsbt)
    # xsbt = xsbt.replace('<str> " " </str>', 'STR')
    return xsbt

def extract_method_vars(source, node, lang=None):
    xsbt = []
    if len(node.children) == 0:
        if(node.type != 'identifier'):
            pass
        ...
        for child in node.children:
            # print(child.type)
            if child.type != 'method_invocation' and child.type != 'field_access' \
                    and child.type != 'import_from_statement' and child.type != 'import_statement' \
                    and child.type != 'call' and child.type != 'def':
                xsbt += extract_method_vars(source, child, lang)
            else:
                pass
    return xsbt
...
def extract_method_params(source, root, lang):
    """
    ...
    """
    query = LANGUAGE[lang].query(PATTERNS_METHOD_PARAMS[lang][0])
    captures = query.captures(root)
    if captures == []:
        query = LANGUAGE[lang].query(PATTERNS_METHOD_PARAMS[lang][1])
        captures = query.captures(root)
