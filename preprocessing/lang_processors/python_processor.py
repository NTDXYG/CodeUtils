...
#
from preprocessing.lang_processors.tokenization_utils import (
    process_string,
)
...
class PythonProcessor(LangProcessor):
    ...
    def get_param_name(self, function):
        result = []
        assert isinstance(function, str) or isinstance(function, list)
        if isinstance(function, str):
            function = function.split()
        params = ' '.join(function[function.index("(") + 1: function.index(")")])
        params = params.split(',')
        for p in params:
