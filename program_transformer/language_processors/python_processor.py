...
class PythonProcessor:
        ...
        while len(queue) > 0:
            ...
            if (str(current_node.type) in ['+=', "-=", "*=", "/=", "%="]) and "assignment" in str(current_node.parent.type):
                ...
                if len(nodes) == 3 and ("identifier" in str(nodes[
                                               0].type) or "integer" in str(nodes[
                                               0].type)):  # this line is to double check whether the
                    # unary operation happens inside an assinemnt expression
                    post_expr.append(current_node.parent.parent)
                    # else:
                    #     pre_expr.append(current_node.parent.parent)
            for child in current_node.children:
                queue.append(child)
    ...
    def post_incre_decre_removal(cls, code_string, post_expr, root, parser):
        ...
        children = root.children
        if len(children) == 0:
            tokens.append(code_string[root.start_byte:root.end_byte].decode())
        for child in children:
